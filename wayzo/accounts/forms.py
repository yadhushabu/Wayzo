...
from restaurants.models import SECTION_CHOICES
...
class AgencySignUpForm(UserCreationForm):
    ...
    place_id_proof = forms.FileField()
    address = forms.CharField(widget=forms.Textarea)
    mobile = forms.CharField()
    city=forms.CharField()
    state=forms.CharField()
    ...
    def save(self, commit=True):
        ...
        if commit:
            ...
            AgencyProfile.objects.create(
                ...
                place_id_proof=self.cleaned_data['place_id_proof'],
                address=self.cleaned_data['address'],
                mobile=self.cleaned_data['mobile'],
                city=self.cleaned_data['city'],
                state=self.cleaned_data['state'] 
...
def save(self, commit=True):
    ...
    if commit:
        ...
        restaurantprofile = RestaurantProfile.objects.create(
            user=user,
